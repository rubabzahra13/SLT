...
import type { PayrollMailDraft } from "@/lib/producer-payroll-mail";

export type PayrollSendPreviewItem = {
  producerName: string;
  email: string;
...
type PayrollSendMailModalProps = {
  open: boolean;
  onClose: () => void;
  items: PayrollSendPreviewItem[];
...
function PayrollMailTablePreview({ rows }: { rows: ProducerFacingPayrollRow[] }) {
  return (
      ...
      <table className="w-full min-w-[960px] border-collapse text-left">
        <thead>
          <tr className="border-b border-brand-line/60 bg-brand-signature/8">
            {PRODUCER_STATEMENT_COLUMNS.map((column) => (
              <th
                key={column.key}
        ...
        <tbody className="divide-y divide-brand-line/40">
          {rows.length === 0 ? (
            <tr>
              <td
                colSpan={PRODUCER_STATEMENT_COLUMNS.length}
                  ...
                  return (
                    <td
                      ...
                      className={clsx(
                        ...
                        isTotal && "font-semibold text-brand-success",
                        !isProgram && !isTotal && "text-brand-ink-secondary"
                      )}
                    >
                      {value}
                    </td>
    ...
    <div className="overflow-hidden rounded-2xl border border-brand-line/70 bg-white shadow-[var(--shadow-premium-sm)] ring-1 ring-inset ring-brand-line/15">
      <div className="bg-gradient-to-br from-brand-signature to-brand-blue px-5 py-5">
        ...
        <p className="mt-1 text-[13px] text-white/90">
          {draft.toName} · {draft.periodLabel}
        </p>
      </div>
      ...
      <div className="space-y-4 px-5 py-5">
        ...
        <PayrollMailTablePreview rows={rows} />
...
export function PayrollSendMailModal({
  open,
  onClose,
  items,
  gmailFrom,
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="payroll-send-mail-title"
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <h2
                  id="payroll-send-mail-title"
                  className="mt-0.5 text-[18px] font-semibold tracking-[-0.02em] text-brand-ink"
                >
                  {sent ? "Statements delivered" : "Payroll email preview"}
                </h2>
                <p className="mt-1 text-[13px] text-brand-ink-secondary">
                  {sent
                    ? sentSummary
                    : multiple
                      ? "Each editor receives their own email with the statement table and an Excel attachment."
                      : "Confirm the statement table and Excel attachment before sending."}
                </p>
              </div>
            </div>
            <button
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          {sent ? (
            ...
            <>
              ...
              {activeItem ? (
                <PayrollMailPreview draft={activeItem.draft} rows={activeItem.rows} />
              ) : null}
            </>
          )}
        </div>
