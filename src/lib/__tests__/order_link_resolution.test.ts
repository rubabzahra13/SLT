...
describe("Order Resolution & Link Fix Unit Tests", () => {
  it("findLinkedOrder resolves order by legacy_id when order.id is a UUID", () => {
    ...
    assert.equal(pricing.customerFacingPrice, 2000); // $2000 base package price
    assert.equal(pricing.payrollBasePrice, 2000); // Titanium non-compliant payroll base price is $2000
