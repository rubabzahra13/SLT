...
exports.InlineCell = InlineCell;
exports.InlineSelect = InlineSelect;
exports.InlineCheckOptionGroup = InlineCheckOptionGroup;
exports.InlineTriStateCheckGroup = InlineTriStateCheckGroup;
exports.InlineTwoStateToggle = InlineTwoStateToggle;
exports.InlineMultiCheckGroup = InlineMultiCheckGroup;
...
const inlineControlClass = "h-8 w-full rounded-lg border border-brand-line/60 bg-white px-2.5 text-[12px] font-medium text-brand-ink shadow-[0_1px_1px_rgba(15,30,45,0.04)] outline-none transition-colors hover:border-brand-line-strong focus:border-brand-blue/50 focus:ring-2 focus:ring-brand-blue/15";
/** Keeps inline table controls aligned; optional footer below the control. */
function InlineCell({ children, footer, className, centered = false, }) {
...
function InlineSelect({ value, options, onChange, className, centered = false, readOnly = false, }) {
    ...
    const [activeIndex, setActiveIndex] = (0, react_1.useState)(() => Math.max(0, options.indexOf(value)));
    ...
    return ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsxs)("button", { ref: triggerRef, type: "button", disabled: readOnly, "aria-haspopup": "listbox", "aria-expanded": open, onClick: (e) => {
                ...
                ? (0, react_dom_1.createPortal)((0, jsx_runtime_1.jsx)("div", { ref: menuRef, role: "listbox", onClick: (e) => e.stopPropagation(), onMouseDown: (e) => e.stopPropagation(), className: "fixed z-[60] overflow-y-auto rounded-xl border border-brand-line/60 bg-white p-1 shadow-[var(--shadow-premium)] ring-1 ring-inset ring-brand-line/10 scrollbar-hide", style: {
                        ...
                        return ((0, jsx_runtime_1.jsxs)("button", { type: "button", role: "option", "aria-selected": selected, onMouseEnter: () => setActiveIndex(index), onClick: (e) => {
                                e.stopPropagation();
                                commit(opt);
                            }, className: (0, clsx_1.default)("flex w-full items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 text-left text-[12px] font-medium transition-colors", selected
                                ? "bg-brand-blue-soft text-brand-signature"
                                : active
                                    ? "bg-brand-bg-subtle text-brand-ink"
                                    : "text-brand-ink-secondary hover:bg-brand-bg-subtle"), children: [(0, jsx_runtime_1.jsx)("span", { className: "min-w-0 flex-1 truncate", children: opt }), selected ? ((0, jsx_runtime_1.jsx)(lucide_react_1.Check, { className: "h-3.5 w-3.5 shrink-0 text-brand-signature", strokeWidth: 2.5 })) : null] }, opt));
                    }) }), document.body)
                : null] }));
}
...
function DetailInput({ className, ...props }) {
    return (0, jsx_runtime_1.jsx)(InlineInput, { ...props, className: (0, clsx_1.default)(detailInputClass, className) });
}
function DetailTextarea(props) {
    return (0, jsx_runtime_1.jsx)(InlineTextarea, { ...props, className: detailInputClass });
}
...
function InlineDateInput({ value, onChange, template, min, max, className, readOnly = false, }) {
    ...
    const [mounted, setMounted] = (0, react_1.useState)(false);
    const [position, setPosition] = (0, react_1.useState)(null);
    const normalized = (0, dates_1.toIsoDateString)(value);
    const minIso = (0, dates_1.toIsoDateString)(min) || undefined;
    const maxIso = (0, dates_1.toIsoDateString)(max) || undefined;
    const templateIso = (0, dates_1.toIsoDateString)(template) || undefined;
    const isUnset = !normalized;
    const initialView = parseIsoToLocalDate(normalized || templateIso || "") ?? new Date();
    const [viewMonth, setViewMonth] = (0, react_1.useState)(() => new Date(initialView.getFullYear(), initialView.getMonth(), 1));
    (0, react_1.useEffect)(() => setMounted(true), []);
    const updatePosition = () => {
        const el = triggerRef.current;
        if (!el)
