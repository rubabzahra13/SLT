...
exports.POM_FORM_COLUMN_LABELS = void 0;
exports.inferMusicAffiliate = inferMusicAffiliate;
exports.buildLegacyPackage = buildLegacyPackage;
exports.buildLegacyMusicTheme = buildLegacyMusicTheme;
exports.inferFormType = inferFormType;
exports.inferCheerFormSubtype = inferCheerFormSubtype;
exports.inferDanceFormSubtype = inferDanceFormSubtype;
...
function inferMusicAffiliate(musicTheme) {
    const upper = musicTheme.toUpperCase();
    if (upper.includes("NON COMPLIANT") || upper.includes("NON-COMPLIANT")) {
        return "Non-compliant";
    }
    if (upper.includes("UNLEASH"))
        return "Unleash the Beats Covers";
    if (upper.includes("PM") || upper.includes("POWER"))
        return "Power Music Covers";
    return "Power Music Covers";
}
function buildLegacyPackage(packageType, timeLengthOfMix) {
    ...
    if (!time)
        return pkg;
    if (pkg.toUpperCase().includes(time.toUpperCase()))
        return pkg;
    return `${pkg} ${time}`.trim();
}
function buildLegacyMusicTheme(order) {
    const parts = [
...
function inferCheerFormSubtype(raw) {
    if (raw.formType && raw.formType !== "school-all-star-cheer")
        return undefined;
    if (raw.cheerFormSubtype)
        return raw.cheerFormSubtype;
    const div = (raw.division || "").toLowerCase();
    const cat = (raw.category || "").toLowerCase();
...
function inferDanceFormSubtype(raw) {
    if (raw.formType && raw.formType !== "school-all-star-dance")
        return undefined;
    if (raw.danceFormSubtype)
        return raw.danceFormSubtype;
    const div = (raw.division || "").toLowerCase();
    const notes = (raw.routineNotes || raw.musicTheme || "").toLowerCase();
    const pkg = (raw.packageType || raw.package || "").toLowerCase();
    if (div.includes("pom") || notes.includes("pom") || pkg.includes("pom"))
        return "pom";
    if (div.includes("hip hop") || notes.includes("hip hop") || div.includes("hip-hop")) {
        return "hip-hop";
    }
    if (div.includes("gameday") || notes.includes("gameday"))
        return "gameday";
    if (div.includes("jazz") ||
        div.includes("kick") ||
        notes.includes("jazz") ||
        notes.includes("kick")) {
        return "jazz-kick";
    }
    if (div.includes("team performance") ||
...
function normalizeOrder(raw) {
    ...
    const { tier, limit } = (0, package_1.parsePackage)(raw.package || raw.packageType || "");
    const packageType = raw.packageType?.trim() ||
        tier ||
        raw.package?.split(/\d+:\d+/)[0]?.trim() ||
        raw.package ||
        "";
    const timeLengthOfMix = raw.timeLengthOfMix?.trim() ||
        (limit !== "-" ? limit : "") ||
        "";
    const musicAffiliate = raw.musicAffiliate?.trim() || inferMusicAffiliate(raw.musicTheme || raw.routineNotes || "");
    const routineNotes = raw.routineNotes?.trim() || raw.musicTheme || "";
    const powerMusicCovers = raw.powerMusicCovers?.trim() || "";
    const requestedEditor = raw.requestedEditor?.trim() ||
    ...
    const splitOrNoSplit = raw.splitOrNoSplit?.trim() ||
        (raw.package?.toUpperCase().includes("NO SPLIT")
            ? "No Split"
            : raw.package?.toUpperCase().includes("SPLIT")
                ? "Split"
                : "");
    const songList = raw.songListSuggestions?.trim() || powerMusicCovers || raw.musicTheme || "";
    const normalized = {
        ...raw,
        formType,
        cheerFormSubtype,
        ...
        splitOrNoSplit,
        virocChoreographerName: raw.virocChoreographerName?.trim() ||
            (cheerFormSubtype === "school-cheer-viroc-yes" ? raw.choreographerName?.trim() : "") ||
            "",
        virocChoreographerEmail: raw.virocChoreographerEmail?.trim() ||
...
function displayText(value) {
    if (!value?.trim())
        return "—";
    return (0, data_1.titleCase)(value);
}
function displayMultiline(value, max = 120) {
    if (!value?.trim())
        return "—";
